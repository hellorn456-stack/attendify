...
import { ScanLine, BookOpen, PieChart, User } from 'lucide-react'
...
const TABS=[{id:0,label:'Attendance',icon:ScanLine},{id:1,label:'Lectures',icon:BookOpen},{id:2,label:'Report',icon:PieChart},{id:3,label:'Profile',icon:User}]
const greet=()=>{const h=new Date().getHours();return h<12?'Good morning':h<17?'Good afternoon':'Good evening'}

export default function StudentDashboard() {
  const {userProfile}=useAuth(); const [tab,setTab]=useState(0)
  const profile=userProfile??{firstName:'Rahul',surname:'Aryan',systemId:'STU-00142',rollNumber:'SE-AIDS029',email:'rahul@college.edu',phone:''}
  ...
  return(
    <div style={{background:C.bg,minHeight:'100vh',maxWidth:480,margin:'0 auto',display:'flex',flexDirection:'column'}}>
      <header style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:'14px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:30}}>
        <div><p style={{margin:0,fontSize:12,color:C.muted}}>{greet()},</p><h1 style={{margin:0,fontSize:17,fontWeight:700,fontFamily:F.display,color:C.text}}>{profile.firstName} {profile.surname}</h1></div>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <span style={{fontSize:11,fontWeight:600,background:C.orangePale,color:C.orange,borderRadius:6,padding:'3px 8px'}}>{profile.rollNumber}</span>
      ...
      <main style={{flex:1,overflowY:'auto',paddingBottom:80}}>
        {tab===0&&<MarkAttendance profile={profile}/>}
        {tab===1&&<MyLectures profile={profile}/>}
        {tab===2&&<AttendanceReport profile={profile}/>}
        {tab===3&&<StudentProfile profile={profile}/>}
      </main>
