...
import {
  MapPin, QrCode, Fingerprint, LogIn, UserPlus,
  RefreshCw, Lock, BarChart2, Users, BookOpen, Shield, Check,
} from 'lucide-react'
...
// ─── Decorative floating cards in the hero ────────────────────────────────────
function HeroVisual({ seconds }) {
  return (
    <div className="hero-visual" style={{ position: 'relative', height: 480, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
      {/* warm glow */}
      ...
      <div style={{ position: 'absolute', bottom: 148, left: 4, background: '#fff', borderRadius: 16, padding: '12px 16px', boxShadow: '0 8px 24px rgba(0,0,0,.08)', border: '1px solid #BBF7D0', zIndex: 3 }}>
        ...
        <div style={{ fontSize: 13, fontWeight: 600, color: '#16A34A', display: 'flex', alignItems: 'center', gap: 5 }}>
          <Check size={13} /> SE-AIDS029
        </div>
      </div>
    </div>
  )
...
// ─── Feature data ─────────────────────────────────────────────────────────────
const BIG_FEATURES = [
  { icon: <MapPin size={22} color="#F97316" />, n: '01', title: 'GPS Location', desc: 'Students must be physically within the teacher-defined radius. Verified before any further step.' },
  { icon: <QrCode size={22} color="#F97316" />, n: '02', title: 'Rotating QR Code', desc: 'QR codes refresh every 20–40 seconds. Old codes are invalidated instantly upon rotation.' },
  { icon: <Fingerprint size={22} color="#F97316" />, n: '03', title: 'WebAuthn Passkey', desc: 'Biometric server-challenge verification. Single-use challenges expire in 60 seconds.' },
]

const MINI_FEATURES = [
  { icon: <Users size={17} color="#F97316" />, title: '4 Role System', desc: 'Admin, Teacher, Student, Guest — each with distinct permissions and dashboards.' },
  { icon: <BarChart2 size={17} color="#F97316" />, title: 'Attendance Analytics', desc: 'Per-subject percentages, teacher-wise breakdowns, and overall stats for students.' },
  ...
  { icon: <RefreshCw size={17} color="#F97316" />, title: 'Auto Year Promotion', desc: 'Roll numbers auto-promote on a configurable annual date (default: July 1).' },
  ...
  { icon: <Shield size={17} color="#F97316" />, title: 'Permanent System IDs', desc: 'STU-XXXXX and TCH-XXXXX generated atomically on signup — never reassigned.' },
]

const STEPS = [
  { icon: <MapPin size={20} color="#F97316" />, label: 'Share location', desc: 'Student opens app and fetches GPS.' },
  { icon: <QrCode size={20} color="#F97316" />, label: 'Scan QR', desc: "Points camera at the teacher's QR display." },
  { icon: <Fingerprint size={20} color="#F97316" />, label: 'Biometric auth', desc: 'Device prompts Face ID or fingerprint.' },
  { icon: <Check size={20} color="#fff" />, label: 'Attendance recorded!', desc: 'Server verifies all layers and locks the record.', hi: true },
]
...
export default function LandingPage() {
  ...
  const closModal  = () => setView('landing')
    ...
    <>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '17px 40px', borderBottom: '1px solid #F5F5F4', position: 'sticky', top: 0, zIndex: 40, background: 'rgba(255,255,255,.93)', backdropFilter: 'blur(16px)' }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button className="btn-outline" style={{ padding: '9px 18px', fontSize: 14 }} onClick={openSignup}>Sign Up</button>
          <button className="btn-primary"  style={{ padding: '9px 18px', fontSize: 14 }} onClick={openLogin}>Log In</button>
        </div>
      </nav>

      {/* ── HERO ─────────────────────────────────────────────────────────── */}
      <section style={{ maxWidth: 1080, margin: '0 auto', padding: '84px 40px 60px' }}>
      ...
      <section style={{ maxWidth: 1080, margin: '0 auto', padding: '72px 40px' }}>
        <div style={{ textAlign: 'center', marginBottom: 52 }}>
          <h2 style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontSize: 'clamp(24px, 3vw, 38px)', fontWeight: 700, color: '#1C1917', marginBottom: 12 }}>
            Mark attendance in 4 steps
          </h2>
          <p style={{ fontSize: 15, color: '#78716C', maxWidth: 440, margin: '0 auto', lineHeight: 1.7 }}>
            The whole flow takes under 10 seconds for students.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', maxWidth: 800, margin: '0 auto', position: 'relative' }}>
          ...
          {STEPS.map((s, i) => (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 14 }}>
              <div style={{ width: 56, height: 56, borderRadius: '50%', background: s.hi ? 'linear-gradient(135deg,#F97316,#EA580C)' : '#fff', border: s.hi ? 'none' : '2px solid #FED7AA', display: 'grid', placeItems: 'center', boxShadow: s.hi ? '0 8px 20px rgba(249,115,22,.35)' : '0 2px 8px rgba(0,0,0,.06)', position: 'relative', zIndex: 1 }}>
                {s.icon}
              </div>
              <div>
                <div style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontSize: 14, fontWeight: 700, color: s.hi ? '#F97316' : '#1C1917', marginBottom: 4 }}>{s.label}</div>
      ...
      <section style={{ background: 'linear-gradient(135deg, #F97316 0%, #C2410C 100%)', padding: '72px 40px', textAlign: 'center' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontSize: 'clamp(26px, 3.5vw, 38px)', fontWeight: 700, color: '#fff', marginBottom: 14 }}>
            Ready to secure your campus?
          </h2>
          <p style={{ fontSize: 16, color: 'rgba(255,255,255,.8)', marginBottom: 36, lineHeight: 1.72 }}>
            Sign up as a student and start marking tamper-proof attendance in minutes.
          </p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button
              onClick={openSignup}
              style={{ background: '#fff', color: '#EA580C', border: 'none', borderRadius: 12, padding: '14px 26px', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 4px 16px rgba(0,0,0,.15)' }}
            >
              <UserPlus size={15} /> Create Student Account
            </button>
            <button
              onClick={openLogin}
              style={{ background: 'rgba(255,255,255,.15)', color: '#fff', border: '2px solid rgba(255,255,255,.45)', borderRadius: 12, padding: '12px 26px', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}
            >
              Log In
            </button>
          </div>
      ...
      {/* ── MODALS ───────────────────────────────────────────────────────── */}
      {/* Rendered at the end so they sit on top of everything else */}
